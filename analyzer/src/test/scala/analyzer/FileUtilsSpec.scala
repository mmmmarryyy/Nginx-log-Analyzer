...
class FileUtilsSpec extends AnyFunSuite {
  ...
  test("getMatchedFile should return an array with the correct files for a valid path") {
    val result = FileUtils.getMatchedFile("./analyzer/src/test/scala/analyzer/test_logs/*.txt")
    assert(result.length == 2)
    assert(result.map(_.getName).contains("test_logs.txt"))
    assert(result.map(_.getName).contains("test_logs2.txt"))
  }

  test("getMatchedFile should return an array with the correct files for a recursive path") {
  ...
  test("generateReportFileName should generate a unique filename for each report") {
